...
## finalとは
Javaのfinalとは、修飾子のひとつである。修飾子finalは、それが最後のものであることを表す。  
例えば、円周率のような値は変わることのない定数なので、一度代入されたなら決して変わって欲しくない。そのため、最後の値という意味でfinalを使う。  
変数、クラス、メソッド何でも使える。  

## recordとは
recordの機能を使うと、値の変更が不可能なクラスを簡単に作成できる。  
定義方法は以下のような書き方。  
```
