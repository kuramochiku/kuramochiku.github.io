...
### コンテナプッシュ(ECR)
ECRへのコンテナプッシュについてはECR画面の「プッシュコマンドの表示」からコマンドを取得する。  
1の「認証トークンを取得」は30分単位？で必要のため、基本毎回実施すると考えること。  
2のビルドは上記で実施している場合は省略。  
3は付与したタグ名により前半部分の名前は変わるため注意。  

### イメージからコンテナの起動
-dをつけるとバックグラウンドで起動する。  バックグラウンド起動時に停止する場合はstopコマンドを使用する。  
--rmをつけるとコンテナ停止時にイメージを削除する。  
--nameでイメージに名前を付けることができる。こちらのオプションが無いとランダムの文字列となる。  
```
 $  docker run --name spino-test --rm -p 8080:8080 springio/testrest2
省略
  .   ____          _            __ _ _
...
## コンテナサイドカー
メインコンテナの機能を拡張したり強化したりするサブコンテナのことをサイドカーと呼ぶ。  
2つのコンテナ間では、ネットワークやディスクなどの情報を共有することができる。特徴は以下となる。  
* コンテナ単位でリソースの割り当てが可能。  
* コンテナ単位で障害を封じ込めることができ、全体停止を防止できる。  
* コンテナ単位のデプロイとなり、機能単位でアップグレード、ロールバックが可能である。  
* ログ取得用コンテナなど、使いまわしが可能である。  
